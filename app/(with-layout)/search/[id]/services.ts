...
export async function searchProductById(id: number) {
  try {
    const subqueryHistoricoPrecioVenta =
      createSubqueryUltimoPrecioVentaProducto(id);
    const subqueryHistoricoPrecioCosto =
      createSubqueryUltimoPrecioCostoProducto(id);
    ...
    const productInfoResult = await db
      .select({
        info: {
          ...
          precio_costo: subqueryHistoricoPrecioCosto.precio,
          precio_venta: subqueryHistoricoPrecioVenta.precio,
        },
        zapato: sql<boolean>`${inventarioCategorias.nombre} = 'Zapatos'`,
      })
      ...
      .where(eq(inventarioProductoinfo.id, id));

    if (productInfoResult.length === 0) {
      throw new ValidationError("Producto no encontrado");
    }

    const { info, zapato } = productInfoResult[0];

    let inventario: InventarioSearch[];

    const commonWhere = and(
      eq(inventarioProducto.infoId, id),
    ...
    console.error(e);
    return { data: null, error: "Error al buscar el producto" };
  }
}
